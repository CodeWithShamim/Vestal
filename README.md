...
3 monitor intervals, otherwise **Active**. The staleness check only arms once recurring
wake-ups can actually be expected — a Scheduler task was registered, or the guardian has
heartbeaten after deployment. Until then (e.g. while the Scheduler precompile isn't live on
the testnet) silence reads as Active, not Reviving.
